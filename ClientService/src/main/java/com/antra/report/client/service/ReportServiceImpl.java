...
public class ReportServiceImpl implements ReportService {
    ...
    private void sendDirectRequests(ReportRequest request) {
        ...
        try {
            Future<ExcelResponse> futureExcelResponse = executorService.submit(()->rs.postForEntity("http://localhost:8888/excel", request, ExcelResponse.class).getBody());
        ...
        try {
            Future<PDFResponse> futurePDFRespone = executorService.submit(()->rs.postForEntity("http://localhost:9999/pdf", request, PDFResponse.class).getBody());
            pdfResponse = futurePDFRespone.get();
        } catch(Exception e){
